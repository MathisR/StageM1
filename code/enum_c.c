...
void explore(int k, int n, int mat[], int avail[], int last, long long* c, long long* cfree, int* notpav, int chosen[], int* psqh){
	if (k<N){
		int x,y;
		int nsave = n;
		for (int l=last+1; l<=nsave-1; l++)
		{
			...
			explore(k+1, n, mat, avail, l, c, cfree, notpav, chosen, psqh);
...
int main(int argc, char *argv[])
{
	...
	explore(0, n, mat, avail, last, &count, &countfree, &notpav, chosen, &psqh);
	
	free(avail);
	free(mat);
	free(chosen);

	FILE* sortie = fopen("y.out", "a");	
	fprintf(sortie, "\nN=%d, R=%d\nTotal:\n %lld Fixed Form Polyominos\n %lld Free Form Polyominos\n %d Pseudo-Squares or Pseudo-Hexagons (Beauquier-Nivat Characterisation)\n %d FreeFP NOT paving the plane (for sure)\n\n Unsure for %lld polyominoes even if they passed the test with R=%d", N, R, count, countfree, psqh, notpav, countfree-psqh-notpav, R);
	fclose(sortie);
	return 0;
}
